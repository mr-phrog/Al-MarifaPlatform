...
<head>
   ...
   <title data-ar="مقاطع الفيديو التي أعجبتني" data-en="liked videos"></title>
...
<section class="liked-videos">
   ...
   <h1 class="heading"data-ar="مقاطع الفيديو التي أعجبتني" data-en="liked videos"></h1>
   ...
   <?php
      ...
      if($select_likes->rowCount() > 0){
         $foundContent = false; // Flag to track if any content was found
         while($fetch_likes = $select_likes->fetch(PDO::FETCH_ASSOC)){
            ...
            if($select_contents->rowCount() > 0){
               $foundContent = true; // Content found, set the flag
   ...
   <div class="box">
      ...
      <form action="" method="post" class="flex-btn">
         ...
         <a href="watch_video.php?get_id=<?= $fetch_contents['id']; ?>" class="inline-btn" data-ar="شاهد الفيديو" data-en="watch"></a>
         <input type="submit" value="remove" class="inline-delete-btn" name="remove" data-ar="إزالة" data-en="remove" onclick="return confirmAction (event,'remove_like');">
      </form>
   </div>
   <?php
            }
         ...
         if (!$foundContent) { // Check the flag after the loops
            echo '<p class="empty" data-ar= "لم يتم العثور على المحتوى" data-en="content was not found!" ></p>';      
         }
      }else{
      echo '<p class="empty" data-ar= "لم تتم إضافة أي شيء إلى الإعجابات حتى الآن!" data-en="nothing added to likes yet!"></p>';
   }
   ?>
